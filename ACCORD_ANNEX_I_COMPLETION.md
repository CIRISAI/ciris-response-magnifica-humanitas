# ACCORD ANNEX I — COMPLETION LANGUAGE
**Annex I: Legal & Regulatory Alignment (v 1.2-Beta completion)**
**Status:** RC-candidate completion. Informative; not legal advice.
**Grounding:** *Magnifica Humanitas* (MH) as cited. CIRIS-native voice throughout.
**MISSION.md §1.3 posture:** MH is the senior work; completion language renders its structural claims in CIRIS operational vocabulary. No Catholic-tradition vocabulary imported as CIRIS's own.

---

## How to read this document

Each section below corresponds to a stub or thin section in Annex I v1.1-Beta. The format is:

1. **Encyclical anchor** — verbatim MH citation (key phrase + §).
2. **Completion language** — CIRIS-native operational text (≤200 words).
3. **Implementation hooks** — specific procedures, fields, or constants.

Sections are numbered to match Annex I v1.1-Beta; sub-headings are new completion material only.

---

## §0. Purpose & Scope — Completion
...
**CIRIS completion:** Annex I coverage is not bounded by currently-enacted statute. The federation treats weakening of multilateral regulatory institutions (MH §201) as a compliance-risk factor requiring proactive tracking rather than reactive patching. The Reg-Change Tracker (§6 below) therefore monitors not only enacted law but active international regulatory dialogues — including ITU AI standards processes, OECD AI Policy Observatory outputs, Council of Europe AI Convention ratification status, and UN Secretary-General's AI Advisory Body recommendations — and surfaces material shifts to the WA docket within the "Breaking" escalation path.

**Implementation hook:** `lexwatcher.py` source-feed list MUST include at minimum: EUR-Lex, Federal Register API, ISO ballot tracker (existing), plus `itu.int/en/ITU-T/AI`, `oecd.ai`, `coe.int/ai`, and `un.org/techenvoy`. Federation-level monitoring participation is a first-class compliance obligation, not a roadmap item.
...
**CIRIS completion:** CIRIS deployments that include network-facing inference, API exposure, or federation transport are subject to emerging cyber-domain treaty obligations even where no enacted statute currently applies. The `CYBER_OFFENSIVE` prohibition (ACCORD §I Ch1, prohibitions.py) is the internal firebreak; Annex I §6 tracks the external treaty surface. Where the WA docket receives a "Breaking" tag related to cyber-domain treaty ratification (e.g., Budapest Convention extension, proposed UN cybercrime convention), the CRE Protocol (Annex D) must re-evaluate any ST ≥ 3 deployment with network-facing components before the next F-Audit cycle.

---

## §1. Data-Protection Cross-Walk ("DP-Map") — Completion
...
**CIRIS completion:** The DP-Map table in Annex I v1.1-Beta maps individual data-subject rights to GDPR articles and CIRIS clauses. MH §105 requires that the accountability chain be traceable at every stage — design, deployment, and decision. The following additions complete that chain:
...
**CIRIS completion:** MH §104 names the design-time bias problem that GDPR Art. 35 DPIA and EU-AI-Act Art. 9(7) address procedurally. The DP-Map must include:
...
## §2. Data-Subject Rights (DSR) Hooks — Completion
...
**CIRIS completion:** DSR infrastructure must expose the reason-code behind any automated determination, not merely confirm that a determination was made. The existing `POST /dsr` endpoint with `{right, identifier, scope}` is extended:
...
## §3. Sector-Specific Overlays — Completion
...
### 3.2 Extended sector table

The existing four-sector table is extended with labor-protection and family-stability overlays derived from MH §§148-156 and §§165-169:
...
*Products entering any new sector MUST attach `overlay.yaml` in release PR. Labor/HR, Gig/Platform, and Youth overlays additionally require a `worker_impact_assessment` or `youth_impact_assessment` section in the Creator Intent Statement.*

### 3.3 Jurisdictional WA quorum requirements

*MH §109:* "To speak of subsidiarity calls for protecting the ability of communities to make choices and corrections, rather than having decisions imposed on them from above."

**CIRIS completion:** WA quorums for sector overlay governance are jurisdiction-stratified:
...
## §4. Product-Safety & AI-Act Alignment — Completion

### 4.1 Transparency at every stage, not just at output
...
**CIRIS completion:** The existing EU-AI-Act alignment table covers output-layer transparency (Art. 13) and human oversight (Art. 16). MH §§105–106 require traceability at each internal stage. The following additions complete the EU-AI-Act alignment:

| EU-AI-Act Article | Risk Level | CIRIS Mapping | MH Anchor | Completion gap filled |
|---|---|---|---|---|
| Art 9 Risk Mgmt | High-risk | Section II PDMA + Annex D CRE | MH §105 | — existing — |
...
| Art 15 Robustness | High-risk | Annex G RS ≥ 0.97 | — | — existing — |
...
| Art 14(4) Human Oversight (labor) | High-risk (Annex III §4) | Labor/HR overlay (§3.2 above) | MH §§148–152 | HR/hiring deployments must surface `worker_notice_sent` boolean in CEP |
...
| Art 61 Post-Market Monitoring | High-risk | F-Audit every 24 mo | MH §106 | Monitoring plan must name the feed sources from §0.1 above; "nothing to monitor" is not a valid monitoring plan |

### 4.2 ISO/IEC 42001:2023 alignment

*MH §107:* "A more moral AI is not enough if that morality is determined by a few. What is needed is a more active political involvement that is capable of…"

**CIRIS completion:** ISO/IEC 42001 §6.1 (AI risk treatment) and §9.1 (monitoring and measurement) align with CIRIS as follows:
...
- ISO 42001 §8.4 (AI system impact assessment) → Creator Intent Statement §§ on worker_impact_assessment and youth_impact_assessment (§3.2 above).

---

## §5. Liability Matrix — Completion

### 5.1 Accountability at every stage: design, deployment, decision
...
**CIRIS completion:** The existing liability matrix covers five failure vectors. MH §105 requires the matrix to span design, deployment, and decision stages explicitly. Extended matrix:
...
## §6. Reg-Change Tracker — Completion
...
**CIRIS completion:** MH §§201, 221, 226 establish that passive compliance with enacted law is insufficient when the multilateral institutions that produce law are themselves weakened. The Reg-Change Tracker must be extended from a reactive tool (track enacted changes) to an active participation mechanism:
...
**Federation-level participation (new):**

The CIRIS federation is not merely a compliance recipient. MH §§219–221 name dialogue and negotiation as the primary method of coexistence. In CIRIS operational terms, this means:
...
## §7. Compliance Evidence Pack (CEP) — Completion

### 7.1 Accountability chain documentation
...
**CIRIS completion:** The existing CEP (seven components) is extended to close the accountability chain MH §105 requires:

Every F-Audit (Annex H) MUST export a CEP zip containing:

1. `dp-map.yaml` — live cross-walk **including `controller_si_threshold` field and `bias_audit_ref` pointer** (§1.1 above).
...
8. *(New)* **Reg-dialogue participation record** — WBD entries for any regulatory-dialogue submissions in the audit period; "no submissions" is acceptable if no material regulations were under public comment.
9. *(New)* **Contestability completion record** — For all A3-A4 decisions in the audit period: percentage where human-review was requested, percentage where WBD documentation was completed within 15 days; KPI target ≥ 90%.

CEP hashed and uploaded to `/compliance/cep/{version}.zip`; root hash anchored in transparency log.

---

## §8. Inter-Annex Hooks — No new completion required

Existing inter-annex hooks (Annex F → GDPR Art. 22; Annex G TX-6 → GDPR Recital 28; Annex H → EU-AI-Act Art. 61; Annex J → GDPR Art. 15(1)(h)) remain as stated. New hooks added by this completion:
...
- **§5 Liability matrix (new vectors) → Annex E SI**: Youth-design joint liability uses same SI apportionment formula as existing vectors.
- **§7 CEP item 8 (reg-dialogue) → §6 Tracker**: WBD reg-dialogue records are the source for CEP item 8; no separate logging system.

---

## §9. References — Additions

*Additions to existing references (GDPR, CCPA/CPRA, LGPD, HIPAA, GLBA, EU-AI-Act, ISO/IEC 42001, Restatement Third Torts):*
...
*End of Annex I Completion Language. To be merged with Annex I v1.1-Beta text at §§0–9. This document is informative; not legal advice. Jurisdictional legal review required before deployment in any sector covered by §3.2 new overlays.*
